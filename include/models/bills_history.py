...
class BillsHistory(Base):
    ...
    mgn_mode = Column(String(255), nullable=True, default='', comment='保证金模式 isolated：逐仓 cross：全仓 无仓位类型字段，该字段返回 ""')
    inst_id = Column(String(255), nullable=False, default='', comment='产品ID，如 BTC-USDT')
    ord_id = Column(String(255), nullable=True, default='', comment='订单ID 无订单时，该字段返回 ""')
    exec_type = Column(String(255), nullable=True, default='', comment='流动性方向 T：taker M：maker')
    ...
    trade_id = Column(String(255), nullable=True, default='', comment='最新成交ID')
    cl_ord_id = Column(String(255), nullable=True, default='', comment='客户自定义订单ID')
